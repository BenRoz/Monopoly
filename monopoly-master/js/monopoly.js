...
Monopoly.CurrentPlayerBrokeNumber="";
Monopoly.inGamePlayers=0;
Monopoly.PlayerIsBroke=false;
...
Monopoly.test =function(){
var cells= $(".cell");
cells.addClass("player1")
    .removeClass("available")
    .attr("data-owner","player1")
    .attr("data-rent",12);
}
...
Monopoly.updatePlayersMoney = function(player,amount){
    ...
    if (playersMoney < 0 ){
        ...
        Monopoly.CurrentPlayerBrokeNumber = player;
        Monopoly.playerStatus[player.attr('id')]="broke";
        Monopoly.removePlayer(player);
        return Monopoly.playerBroke;
    }
    player.attr("data-money",playersMoney);
    player.attr("title",player.attr("id") + ": $" + playersMoney);
    var playerTempid= player.attr("id");
    var i = playerTempid.split("player")[1];
    $('#playerMoneyText'+i+'').text(player.attr("title"));
    Monopoly.playSound("chaching");
};

Monopoly.removePlayer= function(currentPlayerBroke){
    ...
    var playerTempid= currentPlayerBroke.attr("id");
    //clearing property belonged to player
    var allProperties = $('.'+playerTempid+'');
        allProperties.removeClass(playerTempid)
            .addClass("available")
            .attr("data-rent","")
            .attr("data-owner","");
    //clearing his money
     var i = playerTempid.split("player")[1];
    $('#playerMoneyText'+i+'').text("Game over");
    //remove player
    currentPlayerBroke.remove();
...
Monopoly.setNextPlayerTurn = function(){
    if (Monopoly.PlayerIsBroke){
        Monopoly.inGamePlayers--;
        var currentPlayerTurn = Monopoly.CurrentPlayerBrokeNumber ;
        if (Monopoly.playerStatus["player"+Monopoly.numOfPlayers]=="broke"){
           Monopoly.numOfPlayers--;
        }
        if (Monopoly.inGamePlayers==1){
            var popup = Monopoly.getPopup("winner");
            popup.find("button").unbind("click").bind("click",function(){
                Monopoly.handleAction(player,"win");
            });
    ...
    if (Monopoly.doubleCounter == 0||Monopoly.playerStatus["player"+playerId]=="broke"){
          var nextPlayerId = playerId + 1;
           Monopoly.doubleCounter = 0;
        if (nextPlayerId > Monopoly.numOfPlayers){
            nextPlayerId = 1;
        }
    }else if (Monopoly.doubleCounter!=0 && !currentPlayerTurn.hasClass(".jailed")) {
        var nextPlayerId = playerId;
    }
    while(Monopoly.playerStatus["player"+ nextPlayerId]=="broke"){
         nextPlayerId++;
         if (nextPlayerId > Monopoly.numOfPlayers){
            nextPlayerId = 1;
        }
...
Monopoly.initPopups = function(){
    var numOfPlayers =0;
    $(".popup-page#intro").find("button").click(numberOfplayerfunc);
    function numberOfplayerfunc(){
        numOfPlayers = $(this).closest(".popup-page").find("input").val();
        if (Monopoly.isValidInput("numofplayers",numOfPlayers)){
            Monopoly.createPlayers(numOfPlayers);
            Monopoly.closePopup();
            Monopoly.numOfPlayers =  parseInt(numOfPlayers);
            Monopoly.inGamePlayers=Monopoly.numOfPlayers;
        }
    };
};
...
Monopoly.createPlayers = function(numOfPlayers){
    ...
    for (var i=1; i<= numOfPlayers; i++){
        ...
        Monopoly.playerStatus["player" + i]="Playing";
        var moneyAmountDiv = $('<div/>').attr('class',"playerMoneyText").attr('id',"playerMoneyText"+i+"").appendTo($(".moneyScore"));
        moneyAmountDiv.text(player.attr("title"));
...
Monopoly.getNextCell = function(cell){
    ...
    if (typeof currentCellId === 'undefined'){
        var nextCellId = currentCellId;
    }else{
...
Monopoly.adjustBoardSize = function(){
    ...
    boardSize = boardSize- (parseInt(gameBoard.css("margin-top"))+50 )*2;
